...
class WordBlock:
    ...
    def _checkSubsets(self, winSize, posList, wordCount, wordPos, currentWord):
        #print "checkSubsets", posList, wordCount, wordPos, currentWord
        #for w,count in words.iteritems():
        # for all possible contigous subsets of this word
        #for i in range(0, (len(b.words[w])-count)+1):
        ...
        maxPos = max(pos)
        minPos = min(pos)
        if maxPos - minPos < winSize:
            return True
        else:
            # advance: use next wordPos or increase currentWord 
            if wordPos[currentWord]>=len(posList[currentWord]) - wordCount[currentWord]:
                currentWord+=1
                if currentWord==len(wordCount):
                    return False
            else:
                wordPos[currentWord]+=1
            self._checkSubsets(winSize, posList, wordCount, wordPos, currentWord)
    ...
    def _oneSubsetWithinWindow(self, words, b, winSize): 
        ...
        wordPos   = [0] * len(words) # the pos of this word in posList
        ...
        return self._checkSubsets(winSize, posList, wordCount, wordPos, 0)
