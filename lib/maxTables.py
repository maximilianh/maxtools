...
import sys, textwrap, operator, types, logging, re, os
from types import *

# external packages
import namedtuple, util
...
###
class BlockReader:
    """ a parser for sorted, tables: Splits a table 'blocks' where values in one field are identical. Returns one block at a time. As such, it can process GB of 
    tables, without having to load them into memory. Tables MUST BE sorted with sort: remember to use the -n option, do not pipe into sort to save memory. """
...
class TableParser:
    ...
    def parseTuple(self, tuple):
        ...
        if len(tuple)<len(self.headers):
            logging.warn("not enough fields in line %s. Filling up with empty fields" % tuple)
            diff = len(self.headers)-len(tuple)
            tuple.extend([""] * diff)
...
def sqlConnection(connString=None, **kwargs):
    ...
    >>> conn = sqlConnection(connString="localhost,max,testqwe,temp")
    """
    if connString:
...
def parseDict(fname, comments=False, keyField=0, valField=1, doNotCheckLen=False, otherFields=False, headers=False, keyType=None, valueType=None, errorSummary=False, inverse=False):
